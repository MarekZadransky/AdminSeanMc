...
function Invoke-PrereqCheck {
    ...
    Param (
        ...
        [string[]]$PrereqModules = @('AzureAD','MSAL.PS','ImportExcel','ExchangeOnlineManagement')
    )
    ...
    if (($ExecutionPolicy -ne 'RemoteSigned') -and ($ExecutionPolicy -ne 'Unrestricted')){
        if ($IsAdmin) {
            Write-Host "Execution Policy is '$ExecutionPolicy', setting to 'RemoteSigned'" -ForegroundColor Yellow
            Set-ExecutionPolicy -ExecutionPolicy RemoteSigned
        }else{
            Write-Host "Execution Policy is '$ExecutionPolicy', start PowerShell as Administrator to set ExecutionPolicy to RemoteSigned." -ForegroundColor Yellow
            break
        }
    }else {
    ...
    foreach ($Module in $PrereqModules) {
        if (Get-Module $Module -ListAvailable) {
            ...
            Write-Host "Module '$Module' not installed, attemping to Install" -ForegroundColor Yellow  
            if ($IsAdmin) {
...
function Get-RandomPassword {
    ...
    }until(((-join $Result) -cmatch '[a-z]') -and ((-join $Result) -cmatch '[A-Z]') -and ((-join $Result) -match '\d'))
...
$CertPwd = Get-RandomPassword
$Thumbprint = New-AadApplicationCertificate -ClientId $appReg.AppId -CertificatePassword $CertPwd -AddToApplication -CertificateName "Tenant Assessment Certificate"
...
$Out = [PSCustomObject]@{   TenantId = $TenantID
                            ClientId = $appReg.appID
                            CertificateThumbprint = $Thumbprint}
$Out
$Out | ConvertTo-Json | Out-File .\AppInfo.json
